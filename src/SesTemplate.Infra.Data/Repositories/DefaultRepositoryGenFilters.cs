...
namespace SesTemplate.Infra.Data.Repositories;
    ...
    where TFilter : Filter
{
    protected virtual Expression<Func<TEntity, bool>> GetFilters(TFilter filter)
    {
        ...
        foreach (PropertyInfo filterProperty in filterProperties)
        {
            ...
            if (filterValue != null)
            {
                PropertyInfo? entityProperty = typeof(TEntity).GetProperty(filterProperty.Name.Replace("From","").Replace("To",""),
    ...
    protected virtual Expression<Func<TEntityInternal, bool>> GetFiltersInternal<TEntityInternal, TFilterInternal>(TFilterInternal filter)
    {
        ...
        foreach (PropertyInfo filterProperty in filterProperties)
        {
            ...
            if (filterValue != null)
            {
                PropertyInfo? entityProperty = typeof(TEntityInternal).GetProperty(filterProperty.Name.Replace("From","").Replace("To",""),
                    BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
